...
class ReplayBuffer(object):
    ...
    def add(self, pc_state, joint_state, con_action, conti_para, next_pc_state, next_joint_state, reward, done):
        if self.size < self.max_size:
            self.buffer.put((pc_state, joint_state, con_action, conti_para, next_pc_state, next_joint_state, reward, done))
            self.size += 1
        else:
    ...
    def sample(self, batch_size):
        ...
        pc_state_batch, joint_state_batch, con_action_batch, conti_para_batch, next_pc_state_batch, next_joint_state_batch, reward_batch, done_batch = zip(*batch)
    ...
    def __getitem__(self, idx):
        pc_state, joint_state, con_action, conti_para, next_pc_state, next_joint_state, reward, done = self.buffer.queue[idx]
    ...
    def load_data(self, filename):
        ...
        arrays = ['pc_state', 'joint_state', 'con_action','conti_para', 'next_pc_state', 'next_joint_state', 'reward', 'done']
