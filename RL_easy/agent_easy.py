...
import time
import datetime
from networks_easy import Feature_extractor, GaussianPolicy, ConditionalPredictNetwork, QNetwork
from actor_easy import ActorWrapper012
...
class AgentWrapper(object):
    ...
    def select_action(self, pc_state, joint_state, explore_rate=0):
        ...
        with torch.no_grad():
            ...
            # print(f"all_one_tensor: {all_one_tensor}, all_minus_one_tensor: {all_minus_one_tensor}")
        ...
        return action_recon.detach().squeeze().cpu().numpy(), conti_para.detach().squeeze().cpu().numpy()
    ...
    def critic_train(self, batch_size, timestep, ratio=None):
        ...
        for _ in range(self.policy_loop_time):
            ...
            with torch.no_grad():
                ...
                action_z, _, _ = self.cvae.encode(feat_encoder, self.conti_action)

            # # # reassign part
            self.reassign_continue(self.conti_para, feat_encoder,
            ...
            current_q1, current_q2, _ = self.critic(feat_critic, self.conti_emb)
            ...
            if timestep % self.policy_freq == 0:
                ...
                conti_emb = self.policy(feat_policy)

                conti_para = self.rescale_to_new_crange(conti_emb)

                # calculate the distance between the reconstruct action and the data's action
                action_recon, _ = self.cvae.decode(feat_encoder, conti_para)
                if expert_batch_size != 0:
                ...
                q1, q2, _ = self.critic(feat_policy, conti_emb)
