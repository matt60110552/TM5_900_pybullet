...
import sys
import time
from networks import Feature_extractor, GaussianPolicy, ConditionalPredictNetwork
...
class AgentWrapper(object):
    ...
    def get_critic(self):
        critic = GaussianPolicy().to(self.device)
    ...
    def critic_train(self, batch_size):
        """
        ...
        (pc_state, joint_state, conti_action,
         dis_action, next_pc_state, next_joint_state,
         reward, done) = self.replay_buffer_id.sample.remote(batch_size)

        if done == 0:
            with torch.no_grad():
                next_all_feat = self.get_feature_for_policy(next_pc_state, next_joint_state)
                target_Q = self.get_target_q_value(next_all_feat)
                target_Q = reward + self.gamma * target_Q
        else:
            target_Q = reward

        current_q1, current_q2, _ = self.critic(next_all_feat, conti_action, dis_action)
        ...
        if self.timestep % self.policy_freq == 0:
            all_feat = self.policy_feat_extractor(pc_state, joint_state)
            policy_loss = -self.critic(all_feat, self.policy(all_feat)).mean()
