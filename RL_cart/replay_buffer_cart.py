...
class ReplayBuffer(object):
    ...
    def add(self, goal_pos, cur_gripper_pos, scene_pointcloud, conti_action,
            next_gripper_pos, next_scene_pointcloud,  reward, done):
        if self.size < self.max_size:
            self.buffer.put((goal_pos, cur_gripper_pos, scene_pointcloud, conti_action,
                            next_gripper_pos, next_scene_pointcloud, reward, done))
            self.size += 1
        else:
    ...
    def sample(self, batch_size):
        ...
        (goal_pos_batch, cur_gripper_pos_batch, scene_pointcloud_batch, conti_action_batch,
         next_gripper_pos_batch, next_scene_pointcloud_batch, reward_batch, done_batch) = zip(*batch)
        return (
    ...
    def __getitem__(self, idx):
        (goal_pos, cur_gripper_pos, scene_pointcloud, conti_action, next_gripper_pos,
    ...
    def save_data(self, filename):
        ...
        (goal_pos_list, cur_gripper_pos_list, scene_pointcloud_list, conti_action_list,
         next_gripper_pos_list, next_scene_pointcloud_list, reward_list, done_list) = [], [], [], [], [], [], [], []

        for item in self.buffer.queue:
            (goal_pos, cur_gripper_pos, scene_pointcloud, conti_action,
    ...
    def load_data(self, filename):
        ...
        arrays = ['goal_pos', 'cur_gripper_pos', 'scene_pointcloud',
                  'conti_action', 'next_gripper_pos', 'next_scene_pointcloud', 'reward', 'done']
