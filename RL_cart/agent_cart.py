...
import time
import datetime
from networks_cart import Feature_extractor, GaussianPolicy, QNetwork
from actor_cart import ActorWrapper012
from replay_buffer_cart import ReplayBuffer
...
class AgentWrapper(object):
    def __init__(self, online_replay_buffer_id, replay_buffer_id, train=True, scene_level=True, batch_size=40):
        ...
        self.policy_freq = 2
        self.discount = 0.99
        self.policy_loop_time = 50
        self.beta = 0.00005
        self.noise_scale = 0.01
        self.noise_clip = 0.08

    # def select_action(self, goal_pos, cur_gripper_pos, scene_pointcloud, explore_rate=0):
    #     """
    ...
    #         scene_pointcloud_tmp = self.prepare_data(scene_pointcloud).unsqueeze(0)
    #         all_state = self.get_feature_for_policy(goal_pos_tmp, cur_gripper_pos_tmp, scene_pointcloud_tmp)
    ...
    def select_action(self, goal_pos, cur_gripper_pos, scene_pointcloud, explore_rate=0):
        """
        Select actions for actors
        The state is a combination of point and joint, should be 576
        """
        with torch.no_grad():
            goal_pos_tmp = self.prepare_data(goal_pos).unsqueeze(0)
            cur_gripper_pos_tmp = self.prepare_data(cur_gripper_pos).unsqueeze(0)
            ...
            # Randomly explore part
            noise = explore_rate * self.noise_scale * torch.randn(conti_action.size()).to(self.device)
            
            # Adjust noise ranges for different segments
            noise[:3] = noise[:3].clamp(-0.03, 0.03)
            noise[3:] = noise[3:].clamp(-0.02, 0.02)
            
            conti_action = (conti_action + noise)
    ...
    def get_feature_for_policy(self, goal_pos, cur_gripper_pos, scene_pointcloud):
        state = self.policy_feat_extractor(goal_pos, cur_gripper_pos, scene_pointcloud)
        return state
    ...
    def critic_train(self, batch_size, timestep, ratio=None):
        ...
        for _ in range(self.policy_loop_time):
            if expert_batch_size > 0:
                (goal_state, cur_gripper_pos, scene_pointcloud, conti_action,
                 next_gripper_pos, next_scene_pointcloud,
                reward, done) = ray.get([self.replay_buffer_id.sample.remote(expert_batch_size)])[0]
            elif policy_batch_size > 0:
                (policy_goal_state, policy_cur_gripper_pos, policy_scene_pointcloud, policy_conti_action,
                 policy_next_gripper_pos, policy_next_scene_pointcloud,
                ...
                scene_pointcloud = np.concatenate((scene_pointcloud, policy_scene_pointcloud), axis=0)
                ...
                next_scene_pointcloud = np.concatenate((next_scene_pointcloud, policy_next_scene_pointcloud), axis=0)
            ...
            self.scene_pointcloud = self.prepare_data(scene_pointcloud)
            self.conti_action = self.prepare_data(conti_action)
            self.next_gripper_pos = self.prepare_data(next_gripper_pos)
            self.next_scene_pointcloud = self.prepare_data(next_scene_pointcloud)
            ...
            with torch.no_grad():
                ...
                # get target_q
                next_policy_feat = self.get_feature_for_policy(self.goal_state,
                                                               self.next_gripper_pos, self.next_scene_pointcloud)
                target_Q = self.get_target_q_value(next_policy_feat)
                target_Q = self.reward + (1 - self.done) * self.discount * target_Q

            feat_critic = self.get_feature_for_policy(self.goal_state, self.cur_gripper_pos, self.scene_pointcloud)
            ...
            if timestep % self.policy_freq == 0:
                ...
                all_policy_loss = policy_loss + 100 * bc_loss
