...
class ActorWrapper(object):
    ...
    def policy_move(self, vis=False, explore_ratio=0):
        ...
        while not done:
            ...
            conti_action = ray.get([self.policy_id.select_action.remote(goal_pos, cur_gripper_pose,
                                                                        scene_point, explore_ratio)])[0]

            conti_action = np.append(conti_action, 0)
            obs = self.env.step(conti_action, delta=True, repeat=300)[0]
            ...
            data_list.append((goal_pos, cur_gripper_pose, scene_point, conti_action[:6],
                              next_gripper_pose, next_scene_point, reward, done))


        for i in range(len(data_list)):
            (goal_pos, cur_gripper_pose, scene_point, conti_action[:6],
             next_gripper_pose, next_scene_point,
             reward, done) = data_list[i]
            if reward == 1:
                for i in range(5):
                    ray.get([self.online_buffer_id.add.remote(goal_pos, cur_gripper_pose, scene_point, conti_action[:6],
                                                       next_gripper_pose, next_scene_point, reward, done)])
            else:
                ray.get([self.online_buffer_id.add.remote(goal_pos, cur_gripper_pose, scene_point, conti_action[:6],
                                                   next_gripper_pose, next_scene_point, reward, done)])
    ...
    def expert_move(self, vis=False):
        ...
        for i in range(len(path)):
            ...
            next_gripper_pose = self.get_cur_gripper_pose()
            next_scene_point = np.concatenate((next_obstacle_points, next_manipulator_points),axis=0)
            conti_action = np.array(cur_gripper_pose) - np.array(next_gripper_pose)
        ...
        for i in range(len(data_list)):
            (goal_pos, cur_gripper_pose, scene_point, conti_action[:6],
             next_gripper_pose, next_scene_point, reward, done) = data_list[i]

            if reward == 1:
                for i in range(5):
                    ray.get([self.buffer_id.add.remote(goal_pos, cur_gripper_pose, scene_point, conti_action[:6],
                                                       next_gripper_pose, next_scene_point, reward, done)])
            else:
                ray.get([self.buffer_id.add.remote(goal_pos, cur_gripper_pose, scene_point, conti_action[:6],
                                                   next_gripper_pose, next_scene_point, reward, done)])
    ...
    def approaching_reward(self, distance, orientation):
        if distance <= 0.23 and orientation > 0.85:
            return 1
    ...
    def create_subspace_labels(self, point_cloud, target_center, voxel_size, x_range, y_range, z_range):
        ...
        # channel, depth, width, height, this line is to add channel
        subspace_labels = np.expand_dims(subspace_labels, axis=0)
        
        return subspace_labels


    def combine_voxel(self, voxel_array, voxel_array_next):
        # channel, depth, width, height
        combined_array = np.zeros_like(voxel_array)
        for i in range(voxel_array.shape[1]):
            for j in range(voxel_array.shape[2]):
                for k in range(voxel_array.shape[3]):
                    if voxel_array[0, i, j, k] == 1 or voxel_array_next[0, i, j, k] == 1:
                        combined_array[0, i, j, k] = 1
                    if voxel_array_next[0, i, j, k] == 2:
                        combined_array[0, i, j, k] = 2
                    elif voxel_array_next[0, i, j, k] == 3:
                        combined_array[0, i, j, k] = 3
