...
class Feature_extractor(nn.Module):
    def __init__(self):
        ...
        self.scene_pointcloud = PointNetFeature()

        # Combine all the information
        self.combine = self.get_mlp(512, 512, 512)

    def forward(self, goal_position, gripper_position, scene_pointcloud):
        x1 = self.goal_position(goal_position)
        x2 = self.gripper_position(gripper_position)
        x3 = self.scene_pointcloud(scene_pointcloud)

        # Concatenate the outputs
        x = torch.cat((x1, x2), dim=1)

        # Process the concatenated features through the MLP layer
        arm_output = self.arm_combine(x)


        output = self.combine(arm_output * x3)
...
class GaussianPolicy(nn.Module):
    def __init__(
        ...
        self.max_limit = torch.tensor([0.08, 0.08, 0.08, 0.05, 0.05, 0.05]).to(self.device)
        self.apply(weights_init_)
    ...
    def forward(self, state):
        ...
        conti_action = torch.tanh(self.conti(x2)) * self.max_limit
