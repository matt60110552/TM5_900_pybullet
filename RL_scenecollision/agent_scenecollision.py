...
import time
import datetime
from networks_scenecollision import Feature_extractor, GaussianPolicy, QNetwork
from actor_scenecollision import ActorWrapper012
from replay_buffer_scenecollision import ReplayBuffer
...
class AgentWrapper(object):
    ...
    def select_action(self, goal_pos, joint_state, cur_gripper_pos, scene_points, explore_rate=0):
        """
        ...
        with torch.no_grad():
            ...
            scene_points_tmp = self.prepare_data(scene_points).unsqueeze(0)
            all_state = self.get_feature_for_policy(goal_pos_tmp, joint_state_tmp, cur_gripper_pos_tmp, scene_points_tmp)
    ...
    def get_feature_for_policy(self, goal_pos, joint_state, cur_gripper_pos, scene_points):
        state = self.policy_feat_extractor(goal_pos, joint_state, cur_gripper_pos, scene_points)
        return state
    ...
    def critic_train(self, batch_size, timestep, ratio=None):
        ...
        for _ in range(self.policy_loop_time):
            if expert_batch_size > 0:
                (goal_state, joint_state, cur_gripper_pos, scene_points, conti_action,
                next_joint_state, next_gripper_pos, next_scene_points,
                reward, done) = ray.get([self.replay_buffer_id.sample.remote(expert_batch_size)])[0]
            elif policy_batch_size > 0:
                (policy_goal_state, policy_joint_state, policy_cur_gripper_pos, policy_scene_points, policy_conti_action,
                policy_next_joint_state, policy_next_gripper_pos, policy_next_scene_points,
                ...
                scene_points = np.concatenate((scene_points, policy_scene_points), axis=0)
                ...
                next_scene_points = np.concatenate((next_scene_points, policy_next_scene_points), axis=0)
            ...
            self.scene_points = self.prepare_data(scene_points)
            ...
            self.next_scene_points = self.prepare_data(next_scene_points)
            ...
            with torch.no_grad():
                ...
                next_policy_feat = self.get_feature_for_policy(self.goal_state, self.next_joint_state,
                                                               self.next_gripper_pos, self.next_scene_points)
                target_Q = self.get_target_q_value(next_policy_feat)
                target_Q = self.reward + (1 - self.done) * self.discount * target_Q

            feat_critic = self.get_feature_for_policy(self.goal_state, self.joint_state, self.cur_gripper_pos, self.scene_points)
            ...
            if timestep % self.policy_freq == 0:
                ...
                all_policy_loss = policy_loss + 40 * bc_loss
