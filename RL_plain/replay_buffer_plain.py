...
class ReplayBuffer(object):
    ...
    def add(self, goal_pos, joint_state, cur_gripper_pos, scene_pointcloud, conti_action,
            next_joint_state, next_gripper_pos, next_scene_pointcloud,  reward, done):
        if self.size < self.max_size:
            self.buffer.put((goal_pos, joint_state, cur_gripper_pos, scene_pointcloud, conti_action,
                             next_joint_state, next_gripper_pos, next_scene_pointcloud, reward, done))
            self.size += 1
        else:
    ...
    def sample(self, batch_size):
        ...
        (goal_pos_batch, joint_state_batch, cur_gripper_pos_batch, scene_pointcloud_batch, conti_action_batch,
         next_joint_state_batch, next_gripper_pos_batch, next_scene_pointcloud_batch, reward_batch, done_batch) = zip(*batch)
        return (
            np.stack(goal_pos_batch),
            np.stack(joint_state_batch),
            np.stack(cur_gripper_pos_batch),
            np.stack(scene_pointcloud_batch),
            np.stack(conti_action_batch),
            np.stack(next_joint_state_batch),
            np.stack(next_gripper_pos_batch),
            np.stack(next_scene_pointcloud_batch),
    ...
    def __getitem__(self, idx):
        (goal_pos, joint_state, cur_gripper_pos, scene_pointcloud, conti_action, next_joint_state, next_gripper_pos,
         next_scene_pointcloud, reward, done) = self.buffer.queue[idx]

        data = {
            "goal_pos": np.float32(goal_pos),
            "joint_state": np.float32(joint_state),
            "cur_gripper_pos": np.float32(cur_gripper_pos),
            "scene_pointcloud": np.float32(scene_pointcloud),
            ...
            "next_scene_pointcloud": np.float32(next_scene_pointcloud),
    ...
    def save_data(self, filename):
        ...
        (goal_pos_list, joint_state_list, cur_gripper_pos_list, scene_pointcloud_list, conti_action_list,
         next_joint_state_list, next_gripper_pos_list, next_scene_pointcloud_list, reward_list, done_list) = [], [], [], [], [], [], [], [], [], []

        for item in self.buffer.queue:
            (goal_pos, joint_state, cur_gripper_pos, scene_pointcloud, conti_action, next_joint_state,
             next_gripper_pos, next_scene_pointcloud, reward, done) = item
            goal_pos_list.append(goal_pos)
            joint_state_list.append(joint_state)
            cur_gripper_pos_list.append(cur_gripper_pos)
            scene_pointcloud_list.append(scene_pointcloud)
            ...
            next_scene_pointcloud_list.append(next_scene_pointcloud)
        ...
        scene_pointcloud_array = np.array(scene_pointcloud_list)
        ...
        next_scene_pointcloud_array = np.array(next_scene_pointcloud_list)
    ...
    def load_data(self, filename):
        ...
        arrays = ['goal_pos', 'joint_state', 'cur_gripper_pos', 'scene_pointcloud',
                  'conti_action', 'next_joint_state', 'next_gripper_pos', 'next_scene_pointcloud', 'reward', 'done']
