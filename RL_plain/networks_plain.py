...
class ConditionalPredictNetwork(nn.Module):
    def __init__(
        ...
        self.encode2hidden = self.get_mlp(num_inputs + latent_size, hidden_dim, hidden_dim)
    ...
    def forward(self, state_feat, conti_action, state_pred=True):

        action_z, mean, log_std = self.encode(state_feat, conti_action)
        action_recon, state_next = self.decode(state_feat, action_z, state_pred)
        return action_recon, state_next, action_z, mean, log_std

    def encode(self, state_feat, conti_action):

        x1 = F.relu(self.encode2latent(conti_action))
        x2 = F.relu(self.encode2hidden(torch.cat((state_feat, x1), dim=1)))
        mean = self.mean(x2)
    ...
    def decode(self, state_feat, action_z, state_recon=True):
        ...
        if state_recon:
            return action_recon, state_pred.view(state_pred.size(0), 10, 3)
        else:
