...
import time
import datetime
from networks_plain import Feature_extractor, GaussianPolicy, QNetwork
...
class AgentWrapper(object):
    ...
    def select_action(self, goal_pos, joint_state, cur_gripper_pos, scene_pointcloud, explore_rate=0):
        """
        ...
        with torch.no_grad():
            ...
            all_state = self.get_feature_for_policy(goal_pos_tmp, joint_state_tmp, cur_gripper_pos_tmp, scene_pointcloud_tmp)
    ...
    def get_feature_for_policy(self, goal_pos, joint_state, cur_gripper_pos, scene_pointcloud):
        state = self.policy_feat_extractor(goal_pos, joint_state, cur_gripper_pos, scene_pointcloud)
        return state
    ...
    def critic_train(self, batch_size, timestep, ratio=None):
        ...
        for _ in range(self.policy_loop_time):
            if expert_batch_size > 0:
                (goal_state, joint_state, cur_gripper_pos, scene_pointcloud, conti_action,
                next_joint_state, next_gripper_pos, next_scene_pointcloud,
                reward, done) = ray.get([self.replay_buffer_id.sample.remote(expert_batch_size)])[0]
            elif policy_batch_size > 0:
                (policy_goal_state, policy_joint_state, policy_cur_gripper_pos, policy_scene_pointcloud, policy_conti_action,
                policy_next_joint_state, policy_next_gripper_pos, policy_next_scene_pointcloud,
            ...
            with torch.no_grad():
                ...
                # get target_q
                next_policy_feat = self.get_feature_for_policy(self.goal_state, self.next_joint_state,
            ...
            feat_critic = self.get_feature_for_policy(self.goal_state, self.joint_state, self.cur_gripper_pos, self.scene_pointcloud)
